      ...
      <div style={styles.card}>
        ...
        <div style={styles.controls}>
          <label style={styles.label}>
            <span>Model</span>
            <input value={model} onChange={e => setModel(e.target.value)} placeholder="例如 gemini-2.5-flash" style={styles.input} />
          </label>
          ...
          <label style={styles.label}>
            ...
            }} placeholder="貼上你的 API Key" style={styles.input} />
        ...
        <form onSubmit={e => { e.preventDefault(); sendMessage(); }} style={styles.composer}>
          <div style={styles.inputGroup}>
            <div style={styles.fileInputContainer}>
              <input type="file" accept="image/*" onChange={e => setSelectedFile(e.target.files ? e.target.files[0] : null)} style={styles.fileInput} />
              {selectedFile && <span style={styles.fileName}>已選取檔案: {selectedFile.name}</span>}
            </div>
            <input placeholder={selectedFile ? "輸入文字描述（選填）" : "輸入訊息，按 Enter 送出"} value={input} onChange={e => setInput(e.target.value)} style={styles.textInput} />
          </div>
          <button type="submit" disabled={loading || (!input.trim() && !selectedFile) || !apiKey} style={styles.sendBtn}>{loading ? '等待' : '送出'}</button>
        </form>
